# Results from: conditions-filtering.test.ts, TST-002: Equality condition filters rows
# State machines:
#  - name: m
#    states:
#      - name: s
...
#   `<>`, `>`, `<`, `>=`, `<=`.
# - [REQ-097] The operator shall be taken from AST path `...arguments[*].condition.operator`.
# - [REQ-098] The condition value shall be taken from AST path `...arguments[*].condition.value`.
