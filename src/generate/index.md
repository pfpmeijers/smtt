...
machine in each output directory.
...
| [`steps.ts`](./steps.ts) | `renderStepFiles()` / `writeStepFiles()` — generates one `.steps.js` file per state machine. |
| [`fixtures.ts`](./fixtures.ts) | `renderFixtureFiles()` / `writeFixtureFiles()` — generates one `.fixtures.js` file per state machine plus `fixtures/index.js`. |
